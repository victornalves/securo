...
**Outcome.** Control added to `transaction-dialog.tsx`, gated on `!isSynced`. The three D5 behaviours
are implemented as: a `plannedTouched` ref that the effect checks before mirroring the date, and the
effect itself early-returning unless `isCreating`. So the date drives the default only on creation,
and only until the user touches the control.

`frontend/node_modules` was empty — dependencies had never been installed in this checkout. Ran
`npm ci` so typecheck and tests could actually run rather than being asserted by reading.

All i18n keys for T11–T13 were added in one pass across all nine shipped locales (en, pt-BR, es, fr,
de, it, pl, ru, uk) rather than three, since the repo already has `src/locales/i18n.test.ts` enforcing
key parity — that test covers T14's i18n half for free.

`tsc -b` clean, frontend suite green (52 tests, 4 files).
