...
**Outcome.** Two corrections to this task's own guidance, both found while implementing:

1. **`counts_as_realized()` was the wrong predicate for balance.** It is built on `counts_as_pnl`,
   which drops paired transfers — correct for income/expense (both legs cancel) but wrong for a
   balance, where a transfer genuinely moves money out of the account. Added `is_realized()` to
   `_query_filters`: status-only, no P&L exclusions, no `include_planned` parameter. `counts_as_realized`
   is now `counts_as_pnl() AND is_realized()` and stays for the forecast paths from T5. A regression
   test pins the transfer case, since this is the kind of thing a later refactor would "simplify" back.

2. **Future days in balance history are not observable.** `get_balance_history` renders every day
   after today as `balance=None` for the current month (the `day <= cutoff_day` guard), so the future
   window's contents cannot be asserted through the API. The month is still split into two
   `_daily_deltas` windows — past settled-only, future preference-governed — and the future window is
   tested directly on `_daily_deltas`. **T14's coverage matrix needs its "balance history, future days"
   row corrected accordingly.**

The two-window split also had to keep future *posted* rows counting when the preference is off, which
a naive "only query the future window when include_planned" would have dropped. Covered by a test.

Seven tests in `tests/test_planned_transactions_balance.py`, the settled-balance one parametrized over
both toggle states — that exception is the whole point of the task.

Full suite green — 2415 passed, 7 skipped. Ruff clean.
