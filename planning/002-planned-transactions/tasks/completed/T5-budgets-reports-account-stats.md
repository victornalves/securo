...
**Outcome.** The double-counting guard needed **no new code**. `_get_recurring_projections` already
starts each rule's occurrence walk at `rec.next_occurrence`, and `generate_pending` advances that
cursor past everything it materializes — so a materialized occurrence is never also projected. A
test now pins that behavior so T8 cannot silently break it.

Forecast inputs were switched to `counts_as_realized()` rather than being made toggle-aware:
`_get_baseline_projection` and `_get_earliest_transaction_date`. A planned row must not feed the
historical mean that would then forecast it, and a planned row must not redefine where the user's
history begins. Both hold in either toggle state. `_get_baseline_projection` already bounded by
`date <= today`, but a *past-dated* planned row would have slipped through — only a status check
catches that.

`get_account_summary` had no `user_id` parameter at all, so it takes a resolved `include_planned`
flag from the API, which holds the `User` on `WorkspaceContext`. All other sites read the flag from
the user they already load.

Six tests in `tests/test_planned_transactions_reports.py`: budget actuals, per-account stats and
cash-flow each move by exactly the planned amount when the toggle flips; baseline projection and
earliest-transaction ignore planned even with the toggle **on**; a materialized recurring occurrence
is not also projected.

Full suite green — 2408 passed, 7 skipped. Ruff clean.
