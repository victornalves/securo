...
**Outcome.** Implemented as a `User.include_planned` property rather than an API-threaded
parameter — every dashboard service function already loads the `User` for `primary_currency`, and
`User.primary_currency` is the established precedent for exactly this. `plan.md` ADR amended (1.1.0);
the constraint that matters — `_query_filters` stays pure — is untouched. No API signature changed.

`get_monthly_trend` loaded the user *after* building its aggregate query, so the load was hoisted to
the top of the function.

Seven tests in `tests/test_planned_transactions_dashboard.py`. Each toggle-on assertion checks the
figure moved by *exactly* the planned amount, so a double-count fails. Also covers: unset preference
defaults to False; `PATCH /api/users/me` round-trip does not clobber sibling preference keys (the
copy-then-assign trap); a planned row in another month does not leak in when the toggle is on.

Monthly-trend is tested at the service layer with a SQLite `to_char` UDF — the API-level test is
skipped repo-wide because `to_char` is Postgres-only, and this way the path is actually exercised.

Full suite green — 2402 passed, 7 skipped. Ruff clean.
