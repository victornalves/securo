...
**Outcome.** Defect confirmed empirically before fixing: with a provider balance of 5000 and a
future-dated debit of 500, `_account_balance_at` returned **5500** — inflated by exactly the debit.
A second case returned 5200 instead of 4700. Both fail on the pre-fix code and pass after.

Four tests added in `tests/test_dashboard_service.py`: connected account ignores future rows;
past deltas between cutoff and today are still walked back (guards against over-correcting);
manual-account branch unaffected. Full backend suite green — 2380 passed, 7 skipped. Ruff clean.
