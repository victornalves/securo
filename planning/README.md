...
| --- | ---- | ----- | ---- | ----- | ---- |
| 008 | Feature | Asset detail drawer | [spec](008-asset-detail-drawer/spec.md) · [plan](008-asset-detail-drawer/plan.md) · [tasks](008-asset-detail-drawer/tasks/) | Victor Alves | — |
...
| ID  | Type | Title | Notes |
| --- | ---- | ----- | ----- |
| 009 | Spike | Buy/sell ledger for manual assets | Split out of spec 008, which keeps `manual` and `growth_rule` assets on valuation history and offers them no trade actions. `add_transaction` does not check `valuation_method`, so the API would accept a trade on an apartment — but `recompute_and_cache` then overwrites `units`, `average_price`, `purchase_price` and `purchase_date` from the replayed ledger, and skips `_apply_price_to_asset` for non-market assets. Investigate whether the two valuation paths can coexist, or whether a manual asset with a ledger needs a different derivation. Backend question; 008 is frontend-only and cannot resolve it. |
